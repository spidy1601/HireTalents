...
def search_dev(client_skills=['PyThOn','javascript','reactjs']):
    ...
    all_skills_raw = Answer.objects.filter(question_id=19)
    all_skills = all_skills_raw.values_list(Lower('value'),flat=True)
    ...
    selected_dev_id = [all_skills_raw[i].user_answer_id for i, skill_set in enumerate(dev_skills) if all(skill in skill_set for skill in cleaned_skills)]
