    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not request.user.is_authenticated and not survey.can_anonymous_user:
            messages.warning(request, gettext(
                "Sorry, you must be logged in to fill out the survey."))
            return redirect("home")
        ...
        if request.user.is_authenticated and not survey.duplicate_entry and \
            ...
            messages.warning(request, gettext(
                "You have submitted out this survey."))
            return redirect("home")
...
def display(request):
    client_skills = Answer.objects.filter(
        question_id=5).values_list(Lower('value'), flat=True)
    client_skills = client_skills[len(client_skills)-1].split(",")
    nums = search_dev(client_skills)
    if request.method == "POST":
        my_model = ClientDetail(
            selected_ids=request.POST.getlist("selected-id"))
        my_model.save()
...
def detail(request, pk):
    ...
    nums = ClientDetail.objects.filter(detail_id=pk)[0].selected_ids.replace(
...
def inquiries(request):
    ...
    vals = ClientDetail.objects.values_list(
        'detail_id', 'meeting_done').order_by('-detail_id')
    all_companies = []
    for val in vals:
        all_companies.append(Answer.objects.filter(
            user_answer_id=val[0], question_id=12).values('user_answer_id', 'value'))
